...
describe Pronto::ProntoSimplecov::Runner do
  ...
  context 'with patches' do
    let(:patches) {  Pronto::Git::Repository.new('./spec/data/git').diff('master') }
    ...
    it 'returns nothing when unchanged lines are uncovered' do
      files = [SimpleCov::SourceFile.new(File.expand_path('spec/data/git/test.rb'), [0, 0, 0, 1, 1, 1, 1, 1, 1])]
    ...
    it 'only creates messages for uncovered lines' do
      files = [SimpleCov::SourceFile.new(File.expand_path('spec/data/git/test.rb'), [1, 1, 1, 1, 1, 1, 0, 1, 1])]
      expect(SimpleCov::ResultMerger).to receive_message_chain(:merged_result, :files).and_return(files)
    ...
    it 'it irgnores empt lines' do
      files = [SimpleCov::SourceFile.new(File.expand_path('spec/data/git/test.rb'), [1, 1, 1, 1, nil, 1, 1, 1, 1])]
      expect(SimpleCov::ResultMerger).to receive_message_chain(:merged_result, :files).and_return(files)
      expect(runner.run).to eq([])
    end
  end
